...
export async function collectAll({ apiKey, printSql = false } = {}) {
  ...
  const [totalsRows, dailyRows, pagesRows, sourcesRows, countriesRows, eventsRows, funnelRows] = await gather([
    () => hogql(`
    SELECT
    ${perVW((v, w) =>
      `uniqIf(person_id, ${inW(w.days)} AND ${v.cond}) AS v_${v.key}_${w.key},
    ...
    // the home page, how many opened at least one project, and which project
    // each converting visitor opened FIRST. Per-person firsts need a subquery,
    // so this is its own request; the outer SELECT collapses it to one row.
    // The first-click lists stay per-visitor (traffic is tiny) and are tallied
    // in shape(), so the mode is exact.
    () => hogql(`
    SELECT
    ${perVW((v, w) =>
      `countIf(entry_${v.key}_${w.key} = '/') AS landed_${v.key}_${w.key},
    countIf(entry_${v.key}_${w.key} = '/' AND fc_${v.key}_${w.key} != '') AS clicked_${v.key}_${w.key},
    groupArrayIf(fc_${v.key}_${w.key}, entry_${v.key}_${w.key} = '/' AND fc_${v.key}_${w.key} != '') AS fcs_${v.key}_${w.key}`)}
    FROM (
      SELECT person_id,
      ${perVW((v, w) =>
        `argMinIf(coalesce(properties.$pathname, ''), timestamp, event = '$pageview' AND ${inW(w.days)} AND ${v.cond}) AS entry_${v.key}_${w.key},
      argMinIf(coalesce(properties.to, ''), timestamp, event = 'project_open' AND ${inW(w.days)} AND ${v.cond}) AS fc_${v.key}_${w.key}`)}
      FROM events
      WHERE event IN ('$pageview', 'project_open') AND ${since(LONGEST - 1)} ${BOTS}
      GROUP BY person_id
    )`),
  ]);
  ...
  function shape(vk) {
    ...
    const windows = {};
    for (const w of WINDOWS) {
      const fcTally = new Map();
      for (const p of fun[`fcs_${vk}_${w.key}`] ?? []) fcTally.set(p, (fcTally.get(p) ?? 0) + 1);
      windows[w.key] = {
        home_funnel: {
          landed: fun[`landed_${vk}_${w.key}`] ?? 0,
          clicked: fun[`clicked_${vk}_${w.key}`] ?? 0,
          // Each converting visitor counted once, under their first project.
          first_clicks: [...fcTally]
